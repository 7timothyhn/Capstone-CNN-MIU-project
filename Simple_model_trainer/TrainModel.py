...
from datasets.DirectoryIteratorWithBoundingBoxes import (
    DirectoryIteratorWithBoundingBoxes,
)
...
def train_model(
    ...
    number_of_classes = len(
        os.listdir(os.path.join(image_dataset_directory, "training"))
    )
    training_configuration = ConfigurationFactory.get_configuration_by_name(
        model_name, optimizer, width, height, training_minibatch_size, number_of_classes
    )
    ...
    training_steps_per_epoch = np.math.ceil(
        training_data_generator.samples / training_data_generator.batch_size
    )
    ...
    validation_steps_per_epoch = np.math.ceil(
        validation_data_generator.samples / validation_data_generator.batch_size
    )
    ...
    if resume_from_checkpoint:
        ...
        print(
            "Model {0} weights loaded from checkpoint {1}. Training will resume from epoch {2}".format(
                training_configuration.name(), resume_from_checkpoint, initial_epoch
            )
        )
    ...
    early_stop = EarlyStopping(
        monitor=monitor_variable, patience=20, verbose=1, restore_best_weights=True
    )
    ...
    tensorboard_callback = TensorBoard(
        log_dir="./logs/{0}_{1}/".format(
            start_of_training, training_configuration.name()
        )
    )
    ...
    if class_weights_balancing_method is not None:
        ...
        for class_name, class_idx in training_data_generator.class_indices.items():
            print(f"{class_name}: {class_weights.get(class_idx, 1.0):.4f}")

    if class_weights_balancing_method is not None:
        print(
            "Using {0} method for obtaining class weights to compensate for an unbalanced dataset.".format(
    ...
    for i in range(len(misclassified_files)):
        print(
            "\t{0} is incorrectly classified as {1}".format(
                misclassified_files[i], misclassified_files_actual_prediction_classes[i]
            )
        )
    ...
    training_result_image = "{1}_{0}_{2:.1f}p.png".format(
        training_configuration.name(), start_of_training, classification_accuracy * 100
    )
    TrainingHistoryPlotter.plot_history(history, training_result_image)

    datasets_string = str.join(",", datasets)
    notification_message = (
        "Training on {0} dataset with model {1} finished. "
        "Accuracy: {2:0.5f}%".format(
            datasets_string, model_name, classification_accuracy * 100
        )
    )
    if send_telegram_messages:
        TelegramNotifier.send_message_via_telegram(
            notification_message, training_result_image
        )
    else:
        print(notification_message)

    dataset_size = (
        training_data_generator.samples
    ...
    image_sizes = "{0}x{1}px".format(
        training_configuration.input_image_rows,
        training_configuration.input_image_columns,
    )
    data_augmentation = "{0}% zoom, {1}° rotation".format(
        int(training_configuration.zoom_range * 100),
        training_configuration.rotation_range,
    )
    today = "{0:02d}.{1:02d}.{2}".format(
        start_of_training.day, start_of_training.month, start_of_training.year
    )
    balancing_method = (
        "None"
        if class_weights_balancing_method is None
        else class_weights_balancing_method
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="delete_and_recreate_dataset_directory",
        action="store_false",
    ...
    parser.add_argument(
        "--no_dynamic_learning_rate_reduction",
        dest="dynamic_learning_rate_reduction",
        action="store_false",
    ...
    if flags.datasets == "":
        raise Exception(
            "No dataset selected. Specify the dataset for the training via the --dataset parameter"
        )
    ...
    if flags.delete_and_recreate_dataset_directory:
        ...
        training_dataset_provider.resize_all_images_to_fixed_size(
